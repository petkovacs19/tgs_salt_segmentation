...
def main(args):
    dataset = TGSDatasetPreprocessor(args.data_path).get_data_with_features()
    folds = StratifiedKFold(args.folds).split(dataset['file'],dataset['class'])    
    for (train_idx, val_idx) in folds:
        ...
        #Creating dataset
        tgs_data = TGSDataset(data_path=args.data_path, batch_size=args.batch_size, seed=args.seed)
        input_shape = mask_shape = (args.target_size, args.target_size)

        train_data_generator = tgs_data.get_data_generator(dataset.loc[train_idx].reset_index(drop=True), input_size=input_shape, mask_size=mask_shape)
        val_data_generator = tgs_data.get_data_generator(dataset.loc[val_idx].reset_index(drop=True), input_size=input_shape, mask_size=mask_shape)
        ...
        train_step_size = len(train_idx) // hvd.size() if args.hvd else len(val_idx)
        val_step_size = len(train_idx) // hvd.size() if args.hvd else len(val_idx)
...
if __name__== "__main__":
    ...
    parser.add_argument('--data_path', type=str, help='Path to data folds', default='/home/pkovacs/tsg/data/all')
